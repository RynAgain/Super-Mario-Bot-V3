...
|   |-- genetic_bruteforce.lua  # GA that evolves button sequences
|   |-- analyze_genome.lua      # Replays winning genome with logging
|   |-- debug_overlay.lua       # Game state overlay for debugging
|   +-- memory_test.lua         # Memory address testing utility
|-- python/
...
Analysis charts are generated by [`scripts/analyze_session.py`](scripts/analyze_session.py) and saved to [`docs/analysis_charts/`](docs/analysis_charts/). The GA brute force + genome analysis provide ground truth for the World 1-1 level layout, which informs the DQN's reward system (pit positions, obstacle locations).

### Training Dashboard

![Training Dashboard](docs/analysis_charts/training_dashboard.png)

### Reward Over Time

![Reward Over Time](docs/analysis_charts/reward_over_time.png)

### Distance Over Time

![Distance Over Time](docs/analysis_charts/distance_over_time.png)

### Episode Length Over Time

![Steps Over Time](docs/analysis_charts/steps_over_time.png)

### Loss and Q-Values

![Loss and Q-Values](docs/analysis_charts/loss_and_qvalues.png)

### Exploration Rate

![Exploration Rate](docs/analysis_charts/exploration_rate.png)

### Action Distribution

![Action Distribution](docs/analysis_charts/action_distribution.png)

### Death Causes

![Death Causes](docs/analysis_charts/death_causes.png)

### Distance Distribution

![Distance Histogram](docs/analysis_charts/distance_histogram.png)

### Zone Survival Rates

![Zone Survival](docs/analysis_charts/zone_survival.png)

### Performance by Episode Bucket

![Bucketed Summary](docs/analysis_charts/bucketed_summary.png)

### Regenerating Charts

To regenerate charts from the latest training session, update the `SESSION` variable in the script and run:

```bash
python scripts/analyze_session.py
```

For text-only analysis (no matplotlib required):

```bash
python scripts/analyze_session_textonly.py
```

Detailed analysis reports are in [`docs/TRAINING_ANALYSIS_20260322_221337.md`](docs/TRAINING_ANALYSIS_20260322_221337.md).

## World 1-1 Level Analysis

Based on GA genome analysis data (`lua/genome_analysis.csv`, `lua/genome_analysis_summary.txt`), the following obstacle map documents the World 1-1 layout as measured from actual playthroughs:

### Obstacle Map

| X Range | Feature | Description |
|---------|---------|-------------|
| 40 | Start | Mario spawn position |
| 352 | Goomba 1 | First enemy |
| 449-600 | Tall Pipes + Goombas | NOT a pit -- pipe obstacles and enemies |
| 722 | Tall Pipe | Major obstacle -- agent gets stuck here frequently |
| 847-933 | Pit 1 | Running jump required -- 86px |
| 939-1029 | Jump zone | Big jump immediately after pit 1 -- 90px, height 118 |
| 1456-1582 | Pit 2 | Largest gap in level -- 126px crossing |
| 2476-2548 | Pit 3 | Late-level gap -- 72px (run killer) |
| 2696-2846 | Staircase | Final staircase approach -- 150px jump |
| 3008-3099 | Flagpole | Level completion -- 91px jump, height 158 |

### Key Findings

- **64 total jumps** needed to complete the level
- **GA completed in 2,680 frames** (44.7 seconds), 9 generations (~450 evaluations)
- The **tall pipe at x=722** is the hardest non-gap obstacle (GA got stuck for 50 genes)
- The **DQN completed the level 3 times** over 6,848 episodes (25 hours) -- slow but genuine learning
- Verified pit positions: **847-933, 1456-1582, 2476-2548** (3 pits total; x~500 is pipes+goombas, not a pit)

## Lua Scripts

| Script | Purpose | Usage |
|--------|---------|-------|
| [`lua/mario_ai.lua`](lua/mario_ai.lua) | Main AI training script -- WebSocket communication with Python trainer | Load in FCEUX during DQN training |
| [`lua/mario_ai_fallback.lua`](lua/mario_ai_fallback.lua) | Simplified fallback for testing without full Python stack | Manual testing |
| [`lua/genetic_bruteforce.lua`](lua/genetic_bruteforce.lua) | Genetic algorithm that evolves button sequences to beat 1-1 (Phase 1: completion, Phase 2: speed optimization with segment splicing) | Standalone -- save state slot 1, load script |
| [`lua/neat_evolve.lua`](lua/neat_evolve.lua) | NEAT neuroevolution -- evolves neural network topology to play SMB reactively (generalizes to level variants) | Standalone -- save state slot 1, load script |
| [`lua/level_randomizer.lua`](lua/level_randomizer.lua) | Randomizes World 1-1 obstacles in RAM (enemy positions, pipe placement) for NEAT training variety | Standalone or called from `neat_evolve.lua` |
| [`lua/analyze_genome.lua`](lua/analyze_genome.lua) | Replays a winning genome with position/action logging | Load after `genetic_bruteforce` produces `winning_genome.txt` |
| [`lua/debug_overlay.lua`](lua/debug_overlay.lua) | Game state overlay for manual play -- position, velocity, features, minimap | Load alongside manual play for debugging |
| [`lua/memory_test.lua`](lua/memory_test.lua) | Memory address testing utility | Development/debugging |
| `lua/test_*.lua` | Various test scripts -- socket, websocket, payload, enhanced memory | Development/debugging |
